...
import expect from '../lib/mocha_proxy.fixt';
import {
  fixt_arrdom_nums,
  fixt_arrdom_range,
  fixt_assertStrings,
...
describe('src/domain.spec', function() {
  ...
  describe('domain_removeGtUnsafe', function() {

    it('edge case 28,29 & 28', function() {
      expect(domain_removeGtUnsafe(domain_createRange(28, 29), 28)).to.eql(domain_createValue(28));
    });

    it('edge case 29,30 & 29', function() {
      expect(domain_removeGtUnsafe(domain_createRange(29, 30), 29)).to.eql(domain_createValue(29));
    });

    it('edge case 30,31 & 30', function() {
      expect(domain_removeGtUnsafe(domain_createRange(30, 31), 30)).to.eql(domain_createValue(30));
    });

    it('edge case 31,32 & 31', function() {
      expect(domain_removeGtUnsafe(domain_createRange(31, 32), 31)).to.eql(domain_createValue(31));
    });
  });
